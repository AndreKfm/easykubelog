...
namespace EndlessFileStreamClasses.Test
{
    ...
    class WriteEntries : IDisposable
    {
        ...
        public WriteEntries(int count)
        {
            Count = count;
            _randDir = new RandomDirectory();
             Endless = new EndlessFileStream(new EndlessFileStreamSettings { BaseDirectory = _randDir.DirectoryPath, MaxLogFileSizeInMByte = 1 });
    ...
    public class EndlessFileStream_PHYSICAL_Tests
    {
        ...
        public void WriteForward_ReadForwardAndCompare()
        {
            ...
            using var w = new WriteEntries(maxLines);
            var e = w.Endless;
